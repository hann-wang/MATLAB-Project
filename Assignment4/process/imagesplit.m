...
figure(4)
total=block_h*block_v
for i=1:block_v
	for j=1:block_h
		subplot(total,i,j)
		v_s=floor(gap_v+(i-1)*w_size+1);
		v_e=floor(gap_v+i*w_size);
		h_s=floor(gap_h+(i-1)*h_size+1);
		h_e=floor(gap_h+i*h_size);
